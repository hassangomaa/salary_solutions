        ...
        <div class="col-md-12">
            @can('image_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.images.create') }}">
                                ...
                                @foreach($images as $key => $image)
                                    <tr data-entry-id="{{ $image->id }}">
                                        ...
                                        <td>
                                            ...
                                            @can('image_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.images.edit', $image->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('image_delete')
                                                <form action="{{ route('frontend.images.destroy', $image->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('image_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.images.massDestroy') }}",
