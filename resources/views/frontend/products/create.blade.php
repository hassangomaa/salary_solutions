                ...
                <div class="card-body">
                    <form method="POST" action="{{ route("frontend.products.store") }}" enctype="multipart/form-data">
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control select2" name="seller_id" id="seller_id" required>
                                @foreach($sellers as $id => $entry)
                                    <option value="{{ $id }}" {{ old('seller_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <input class="form-control" type="text" name="best" id="best" value="{{ old('best', '') }}">
                            @if($errors->has('best'))
                                <div class="invalid-feedback">
                                    {{ $errors->first('best') }}
                                </div>
                        ...
                        <div class="form-group">
                            ...
                            @foreach(App\Models\Product::FEATURED_RADIO as $key => $label)
                                <div>
                                    <input type="radio" id="featured_{{ $key }}" name="featured" value="{{ $key }}" {{ old('featured', '') === (string) $key ? 'checked' : '' }}>
